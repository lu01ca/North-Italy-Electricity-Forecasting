# **Thesis application: Northern Italy Electricity Demand**
...
plt.title("Average impact of holidays on electricity demand (North Zone, 2021-2022)", fontsize = 14, fontweight = 'bold')
...
target_cols_df = [f'H{i}' for i in range(1, 25)]
df_matrix.dropna(subset=target_cols_df, inplace = True)

### Final dataset, removing the first 168 NA due to the embedding ###
...
for i in range(1, 169):
    df_knn[f'Lag_{i}'] = df_knn['Total Load [MW]'].shift(i)

### Seasonal Features ###
df_knn['Hour'] = df_knn.index.hour
df_knn['Day'] = df_knn.index.dayofweek + 1
df_knn['Month'] = df_knn.index.month

### Calendar Effect ###
valid_holidays = [k for k in fixed_holidays.keys() if k != '12-07']
df_knn['Is_Holiday'] = df_knn.index.map(lambda x: 1 if x.strftime('%m-%d') in valid_holidays else 0)
df_knn['Is_Easter'] = df_knn.index.map(lambda x: 1 if x.strftime('%Y-%m-%d') in (easter_dates_str + EM_dates_str) else 0)

### Creation of 24 target variables ###
for i in range(1, 25):
    df_knn[f'H{i}'] = df_knn['Total Load [MW]'].shift(-i)

# Drop last days without target
target_cols_knn = [f'H{i}' for i in range(1, 25)]
df_knn.dropna(subset = target_cols_knn, inplace = True)
...
# Download
# df_knn.to_excel('Final_Dataset_knn.xlsx', index = True)
# files.download('Final_Dataset_knn.xlsx')
...
df_knn = pd.read_excel('Final_Dataset_knn.xlsx')
df_knn.set_index('Date', inplace = True)
...
# MATRIX
target_cols_df = [f'H{i}' for i in range(1, 25)]
feature_cols_df = [c for c in df_matrix.columns if c not in target_cols_df]
...
X_subtrain_df = subtrain_df[feature_cols_df].values
Y_subtrain_df = subtrain_df[target_cols_df].values

X_val_df = val_df[feature_cols_df].values
Y_val_df = val_df[target_cols_df].values

X_fulltrain_df = fulltrain_df[feature_cols_df].values
Y_fulltrain_df = fulltrain_df[target_cols_df].values

X_test_df = test_df[feature_cols_df].values
Y_test_df = test_df[target_cols_df].values
...
# MATRIX KNN
target_cols_knn = [f'H{i}' for i in range(1, 25)]
feature_cols_knn = [c for c in df_knn.columns if c not in target_cols_knn]

# Splitting
subtrain_knn = df_knn.loc[:'2023-12-31 23:00:00']
val_knn      = df_knn.loc['2024-01-01 00:00:00':'2024-12-31 23:00:00']
fulltrain_knn= df_knn.loc[:'2024-12-31 23:00:00']
test_knn     = df_knn.loc['2025-01-01 00:00:00':'2025-12-31 23:00:00']

X_subtrain_knn = subtrain_knn[feature_cols_knn].values
Y_subtrain_knn = subtrain_knn[target_cols_knn].values

X_val_knn = val_knn[feature_cols_knn].values
Y_val_knn = val_knn[target_cols_knn].values

X_fulltrain_knn = fulltrain_knn[feature_cols_knn].values
Y_fulltrain_knn = fulltrain_knn[target_cols_knn].values

X_test_knn = test_knn[feature_cols_knn].values
Y_test_knn = test_knn[target_cols_knn].values

# Check
print("SUB-TRAIN SET:")
print(f"First row: {subtrain_knn.index.min()}")
print(f"Last row: {subtrain_knn.index.max()}")
print(f"X shape: {X_subtrain_knn.shape}")
print(f"Y shape: {Y_subtrain_knn.shape}\n")

print("VALIDATION SET:")
print(f"First row: {val_knn.index.min()}")
print(f"Last row: {val_knn.index.max()}")
print(f"X shape: {X_val_knn.shape}")
print(f"Y shape: {Y_val_knn.shape}\n")

print("FULL-TRAIN SET:")
print(f"First row: {fulltrain_knn.index.min()}")
print(f"Last row: {fulltrain_knn.index.max()}")
print(f"X shape: {X_fulltrain_knn.shape}")
print(f"Y shape: {Y_fulltrain_knn.shape}\n")

print("TEST SET:")
print(f"First row: {test_knn.index.min()}")
print(f"Last row: {test_knn.index.max()}")
print(f"X shape: {X_test_knn.shape}")
print(f"Y shape: {Y_test_knn.shape}")
...
# Sub training set
X_subtrain_df_norm = scaler_X_val.fit_transform(subtrain_df[feature_cols_df].values)
Y_subtrain_df_norm = scaler_Y_sub.fit_transform(subtrain_df[target_cols_df].values)

X_val_df_norm = scaler_X_val.transform(val_df[feature_cols_df].values)
Y_val_df_norm = scaler_Y_sub.transform(val_df[target_cols_df].values)

# Full training set
X_fulltrain_df_norm = scaler_X_test.fit_transform(fulltrain_df[feature_cols_df].values)
Y_fulltrain_df_norm = scaler_Y_test.fit_transform(fulltrain_df[target_cols_df].values)

X_test_df_norm = scaler_X_test.transform(test_df[feature_cols_df].values)
Y_test_df_norm = scaler_Y_test.transform(test_df[target_cols_df].values)

# MATRIX KNN
scaler_X_val_knn = MinMaxScaler()
scaler_X_test_knn = MinMaxScaler()

# Sub training set
X_subtrain_knn_norm = scaler_X_val_knn.fit_transform(subtrain_knn[feature_cols_knn].values)
X_val_knn_norm = scaler_X_val_knn.transform(val_knn[feature_cols_knn].values)

# Full training set
X_fulltrain_knn_norm = scaler_X_test_knn.fit_transform(fulltrain_knn[feature_cols_knn].values)
X_test_knn_norm = scaler_X_test_knn.transform(test_knn[feature_cols_knn].values)
...
X_knn_norm = np.vstack((X_subtrain_knn_norm, X_val_knn_norm))
Y_knn = np.vstack((Y_subtrain_knn, Y_val_knn))

split_index = [-1] * len(X_subtrain_knn_norm) + [0] * len(X_val_knn_norm)
holdout_val = PredefinedSplit(test_fold = split_index)

best_knn_model, knn_metrics = train_knn(
    X_cv = X_knn_norm, Y_cv = Y_knn, X_subtrain = X_subtrain_knn_norm, Y_subtrain = Y_subtrain_knn,
    X_val = X_val_knn_norm, Y_val = Y_val_knn, val_split = holdout_val, n_iter = 200)
